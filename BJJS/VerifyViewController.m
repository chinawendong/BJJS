    ...
    if (alertView == _alert3)
    {
        ...
        if (self.verificationCodeResult) {
            ...
            self.verificationCodeResult (SMSUIResponseStateSuccess,_phone,_areaCode,verifyError);
             ZhuCeViewController *zhu = [[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:@"ZhuCeViewController"];
            zhu.phone = _phone;
            [verifyViewController presentViewController:zhu animated:YES completion:^{
                [verifyViewController.timer2 invalidate];
                [verifyViewController.timer1 invalidate];
            }];
        }
