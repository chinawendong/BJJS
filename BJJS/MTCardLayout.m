...
- (void)useDefaultMetricsAndInvalidate:(BOOL)invalidate
{
    ...
    m.presentingInsets = UIEdgeInsetsMake(10, 0, 44, 0);
    m.listingInsets = UIEdgeInsetsMake(10.0, 0, 0, 0);
    m.minimumVisibleHeight = 64;
	m.flexibleTop = 0.0;
...
//            NSLog(@"%@", NSStringFromUIEdgeInsets(self.collectionView.contentInset));
			attributes.frame = frameForSelectedCard(self.collectionView.bounds, UIEdgeInsetsMake(64, 0, 0, 0), _metrics);
		}
		else
		{
